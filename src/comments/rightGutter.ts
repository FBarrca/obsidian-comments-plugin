...
import { BlockType, WidgetType } from "@codemirror/view";
...
const gutterView = ViewPlugin.fromClass(
	class {
		...
		constructor(readonly view: EditorView) {
			...
			const parent = view.contentDOM.parentNode;
			if (parent) {
				parent.insertBefore(this.dom, view.contentDOM);
			} else {
				view.scrollDOM.appendChild(this.dom);
			}
			if (this.domAfter) this.attachDOMAfter();
		}
		...
		attachDOMAfter() {
			if (!this.domAfter) return;
			const domAfter = this.domAfter;
			const parent = this.view.contentDOM.parentNode;
			if (parent) {
				const next = this.view.contentDOM.nextSibling;
				parent.insertBefore(domAfter, next);
			} else {
				this.view.scrollDOM.appendChild(domAfter);
			}

			this.view.requestMeasure({
				read: () => window.getComputedStyle(this.dom).marginInlineEnd,
				write: (inlineEnd) => {
					domAfter.style.marginInlineEnd = "0";
					if (inlineEnd) {
						domAfter.style.marginInlineStart = inlineEnd;
					} else if (!domAfter.style.marginInlineStart) {
						domAfter.style.marginInlineStart = "0";
					}
					domAfter.style.marginLeft = "";
					domAfter.style.marginRight = "";
				},
			});
		}
		...
		syncGutters(detach: boolean) {
			...
			if (detach) {
				const parent = after ? after.parentNode : this.view.scrollDOM;
				if (parent && after) {
	...
	setMarkers(view: EditorView | null, markers: readonly GutterMarker[]) {
		...
		for (let iNew = 0, iOld = 0; ; ) {
			...
			if (marker.toDOM) {
				...
				else if (view) this.dom.insertBefore(marker.toDOM(view), domPos);
			}
			if (matched) iOld++;
		}
